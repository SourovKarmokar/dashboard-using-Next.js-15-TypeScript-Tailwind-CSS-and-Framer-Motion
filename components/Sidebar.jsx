...
import { Bell, DollarSign, House, Info, Mail, Settings, ShoppingBag, Users ,Menu } from "lucide-react";
...
const Sidebar = () => {
 ...
 useEffect(() => {
  fetch("/data/data.json")
    ...
    .catch((err) => console.error("Error fetching sidebar items:", err));
}, []);
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href}>
                <div
                  className={`flex items-center p-4 text-sm font-medium rounded-lg hover:bg-[#2f2f2f] mb-2 ${
